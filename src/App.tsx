...
export default function App() {
  ...
  const legendItems = useMemo(() => {
    ...
    siteResults.forEach((res) => {
      if (!counts[res.wqiClass]) {
        counts[res.wqiClass] = { color: res.color, count: 0 };
      }
      counts[res.wqiClass].count++;
    });
    return Object.keys(counts)
      .map((wqiClass) => ({
        label: wqiClass,
        color: counts[wqiClass].color,
        count: counts[wqiClass].count,
      }))
    ...
    <div className="min-h-screen bg-[#f4f7fb] text-slate-800 font-sans flex flex-col">
      <header className="bg-gradient-to-r from-slate-900 via-[#0a485c] to-[#04667a] text-white px-6 py-4 flex flex-col md:flex-row items-center justify-between sticky top-0 z-20 gap-4 shadow-lg border-b border-cyan-800/40">
        <div className="flex items-center gap-4">
          ...
          >
            <div className="absolute inset-0 rounded-[20%] mix-blend-overlay border border-[#0000ff] bg-[#e6e6e6]"></div>
            <motion.img
              ...
              className="h-full w-auto object-contain drop-shadow-2xl relative z-10 bg-[#e6e6e6]"
            />
          </motion.div>
          <div>
          ...
          <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-blue-100/60 overflow-hidden flex flex-col h-[400px] transition-shadow hover:shadow-[0_8px_30px_rgba(4,102,122,0.06)]">
            ...
            <div id="map-container-export" className="flex-1 relative z-0">
               <div className="absolute top-4 left-14 z-[1000] bg-white/95 backdrop-blur-md border border-slate-200 p-4 rounded-xl shadow-xl min-w-[220px]">
                       ...
                       <button
                         ...
                         onClick={() => {
                           if (activeFilters.includes(item.label)) {
                             const newFilters = activeFilters.filter(f => f !== item.label);
                             setActiveFilters(newFilters);
                           } else {
                             if (activeFilters.length === 0) {
                               setActiveFilters([item.label]);
                             } else {
                               setActiveFilters([...activeFilters, item.label]);
                             }
                           }
                         }}
                         className={`flex items-center justify-between px-2 py-2 rounded-lg text-sm transition-all ${isActive ? 'bg-slate-50 hover:bg-slate-100 opacity-100' : 'opacity-50 hover:opacity-75 grayscale'}`}
                       >
                         <div className="flex items-center gap-3">
                           <div className="w-4 h-4 rounded-full border border-slate-200 shadow-sm" style={{ backgroundColor: item.color }}></div>
                           <span className="font-semibold text-slate-700">{item.label}</span>
                         </div>
                         <span className="text-xs bg-white border border-slate-100 px-2 rounded-full py-0.5 font-bold shadow-sm text-slate-600">{item.count}</span>
                       </button>
                     );
                   })}
                 </div>
                 {activeFilters.length > 0 && (
                   <button 
                     ...
                     className="mt-3 w-full text-xs text-blue-600 hover:text-blue-800 hover:bg-blue-50 py-2 rounded-md transition-all font-semibold border border-blue-100"
                   >
                     Clear Filters
                   </button>
                 )}
              ...
              >
                ...
                {siteResults.filter(res => activeFilters.length === 0 || activeFilters.includes(res.wqiClass)).map((res, i) => {
