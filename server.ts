...
You are a Water Intelligence Decision Support Engine powered by Gemini AI inside a geospatial environmental analytics platform.`;
      ...
      const contentsArray = (history || []).map((m: any) => ({
        role: m.role,
      ...
      while (retries > 0) {
        try {
          ...
          if (e.status === 503 || e.status === 'UNAVAILABLE' || e.message?.includes('503') || e.message?.includes('demand')) {
            retries--;
            if (retries === 0) throw e;
            // Fallback to a different model on high demand
            if (currentModel === "gemini-3.5-flash") {
              currentModel = "gemini-3.1-flash-lite";
            } else if (currentModel === "gemini-3.1-flash-lite") {
              currentModel = "gemini-3.1-pro-preview";
            }
            console.log(`Model high demand, falling back to ${currentModel}`);
      ...
      return res.status(500).json({ error: message || "An error occurred with the AI assistant. Please try again." });
    }
  });
